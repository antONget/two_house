...
async def yes(message: Message, state: FSMContext):
    ...
    if not current_state:  # Если нет состояния, переходим в выбор ПОДЪЕЗДА
        ...
        await message.answer(text=f"№ дома {number_house}\nВыберите номер вашего подъезда",
                             reply_markup=kb.kb_doorway())
...
async def no(message: Message, state: FSMContext, bot: Bot):
    logging.info(f'no')
    current_state = await state.get_state()
    logging.info(f'current_state = {current_state}')
...
@router.message(lambda massage: massage.text in ['1 этаж', '2 этаж', '3 этаж', '4 этаж', '5 этаж', '6 этаж',
                                                 '7 этаж', '8 этаж'])
...
# Если пользователя нет в БД
    ...
    if not chek_in_user_tg_id:
        ...
        dict_current_state = {'house': data['house'], 'doorway': data['doorway'], 'stage': data['stage'],
                              'flat': data['flat'], 'tg_id': data['tg_id'], 'username': data['username']}

        await rq.add_new_user(data=dict_current_state)
